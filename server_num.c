      ...
      if (select_retval > 0) /* at least one file descriptor is ready */
        {
          ...
          for (current = head.next; current; current = next) {
            ...
            */
                  printf("too much data dump from socket");
                } 
                else {
          ...
          /* something is wrong */
                ...
                short charbuffer = (short) ntohs(*(short *)(buf+2));;
                printf("Received the char \"%d\". Client IP address is: %s\n",
                        charbuffer, inet_ntoa(current->client_addr.sin_addr));
                }
            }
          }
        }
